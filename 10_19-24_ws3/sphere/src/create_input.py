...
def generate_random_input():
    ...
    halfsphere_volume = 4 / 3 * 3 * (r**3) / 2
    cup_volume_mul = random.randint(2, 10)
    cup_volume = halfsphere_volume * cup_volume_mul
    glass_volume = 1
    while ((halfsphere_volume % glass_volume) / halfsphere_volume) * 100 < 1:
        glass_volume = random.randint(2, 1000)
    input_str = '{r}\n{cup_v}\n{glass_v}'.format(r=r, cup_v=int(cup_volume), glass_v=int(glass_volume))
    output_str = "r: %.2f\n" % (r)
    output_str += "Sphere's area: %.2f\n" % (4 * 3 * (r**2))
    output_str += str(cup_volume / halfsphere_volume) + '\n'
    output_str += str(int(((halfsphere_volume % glass_volume) / halfsphere_volume) * 100)) + '\n'
